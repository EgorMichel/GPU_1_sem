...
int main()
{
    Torus const torus[2] =
            {
            {{0.f, +1.0f, 2.0f}, 0.1f,0.2f},
            {{0.f, -1.0f, 2.0f}, 0.3f,0.4f},
    ...
    auto const closestHit = [torus](Ray const ray) noexcept
            -> std::optional<Hit>
    {
        return std::ranges::fold_left
                (
                    torus | std::views::enumerate
                    | std::views::transform([=](auto const &pair) noexcept
                    -> std::optional<Hit>{
                        auto const &[i, s] = pair;
                        return rayIntersection(ray, s).transform
                        (
            ...
            -> vec3
    {
        ...
        vec3 const norm = normalize(pos - torus[i].origin);
        float const NL = std::max(0.f, dot(norm, lightDir));

        auto const shadowHit = closestHit({pos, lightDir});
