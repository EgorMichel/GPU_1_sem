...
auto createBVHImpl(std::span<IBox> const ibox, Boxable &&r) noexcept
{
    ...
    u32 const halfSize = u32(ibox.size() + 1u) / 2u;
    std::ranges::nth_element(ibox, ibox.begin() + halfSize, {}, proj);
    return std::make_unique<BVHNode<T>>
    (
        bigBox,
        typename BVHNode<T>::Kids
        {
            .left  = createBVHImpl(ibox.subspan(0, halfSize), r),
            .right = createBVHImpl(ibox.subspan(   halfSize), r),
        }
    );
}
